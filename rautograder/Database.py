...
class Database:
    ...
    def update_student(self, sunet, field, value):
        return(self.students_db.update_one(
            {"sunet": sunet}, {'$set': {field: value}}))
    ...
    def load_student_info(self, file):
        ...
        for s in sdata.index:
            student_dict = {'sunet': sdata.loc[s, 'SIS Login ID'],
                            'email': '%s@#stanford.edu' % sdata.loc[
                                s, 'SIS Login ID'],
                            'name': s,
                            'id': int(sdata.loc[s, 'SIS User ID'])}
            sunet_matches = [i for i in self.students_db.find(
                {'sunet': student_dict['sunet']})]
